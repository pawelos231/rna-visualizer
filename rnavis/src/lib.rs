...
pub fn make_vis(protein: &Protein) -> Result<(), ()> {
	...
	// TODO: Refactor
	let bases = [
		Tree::from_str(assets::BASE, &options).unwrap(),
		Tree::from_str(assets::BASE_LINK, &options).unwrap(),
		Tree::from_str(assets::BASE_P, &options).unwrap(),
		Tree::from_str(assets::BASE_P_LINK, &options).unwrap(),
	];

	let base_height = bases[0].size.height() as f32;
	let base_width = bases[0].size.width() as f32;
	...
	for acid in acids {
		...
		resvg::render(&bases[0], FitTo::Original, root, map.as_mut());
